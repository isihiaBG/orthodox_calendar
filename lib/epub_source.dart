...
/// Кой запис в съдържанието отговаря на този път — ЧЕТИВОТО, не денят.
///
/// ⚠⚠ ДЕНЯТ И ПЪРВОТО МУ ЧЕТИВО ДЕЛЯТ ЕДИН И СЪЩ ФАЙЛ. Проверено в самите
/// томове: „Памет на 1 август" и „Произхождение на честния и животворящ
/// Кръст" сочат и двете `Text/index_split_641.xhtml`. А [flattened] дава
/// първо РОДИТЕЛЯ, тъй че простото „първия с този href" винаги хващаше деня.
///
/// Оттам идваха две неща наведнъж (докладвани 06.09.2026):
///   • споделеният цитат се именуваше „Памет на 23 август" вместо със
///     светията — и то само когато цитатът е от ПЪРВОТО четиво за деня,
///     затова изглеждаше, че името се мени без правило;
///   • четецът се отваряше на дневния възел, а не на четивото.
///
/// ⚠ Предпочита се ЛИСТ (запис без деца): дневните възли имат деца, самите
/// четива нямат. Няма ли лист — връща се каквото има, вместо `null`.
EpubTocEntry? chapterForHref(EpubBook book, String href) {
  EpubTocEntry? fallback;
  for (final e in book.toc.expand((e) => e.flattened())) {
    if (e.href != href) continue;
    if (e.children.isEmpty) return e;
    fallback ??= e;
  }
  return fallback;
}
