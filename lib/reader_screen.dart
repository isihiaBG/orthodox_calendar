...
class _ReaderScreenState extends State<ReaderScreen>
  ...
  void _goToQuote(ParsedQuoteLink q) {
    ...
    setState(() {
      ...
      // `q.text` е текстът ОТ ЛИНКА: на стар адрес — ОТРЯЗАН, а от версия 3
      // изобщо ПРАЗЕН. Координатите пък носят пълната дължина
      // (`anchor.charLength`/`charEnd` се кодират отделно и без
      // ограничение). Използван директно, q.text подрязваше и маркирането —
      // „на Бога" накрая не светваше, защото него просто го нямаше в
      // изпратения текст.
      //
  ...
  /// Колко се вдига църквицата НАД базовата линия — в дялове от кегела.
  ///
  /// ⚠⚠ СТОЙНОСТТА Е ИЗМЕРЕНА НА ЕКРАННА СНИМКА, не изведена — по
  /// установената в проекта практика за вертикално подравняване (виж „ДВА
  /// КАПАНА В pdf ПАКЕТА": там теоретичните сметки сгрешиха два пъти
  /// подред). Мерено на реда „⛪ 6 апр":
  ///
  ///     цифрата „6"      високa 54 px, долният ѝ ръб е базовата линия
  ///     църквицата       висока 52 px, но долният ѝ ръб беше 23 px НАД нея
  ///     кегелът          54 / 0.687 em (цифрата в Charis SIL) ≈ 78,6 px
  ///
  /// тоест знакът висеше 0,29 em по-нагоре, отколкото трябва. Оттам и
  /// подредбата долу: `Alignment.bottomCenter` слага долния ръб на самата
  /// базова линия (иконата и цифрите заемат ЕДНА И СЪЩА лента — 52 срещу
  /// 54 px), а тази стойност е само оптичен въздух отдолу.
  ///
  /// ⚠ Повдигането е ЧИСТО РИСУВАНЕ (`Transform.translate`) и не влияе на
  /// подредбата; редът остава на мястото си.
  static const double _hramRaise = 0.02;
  ...
  /// Рисунката на църквицата — в дялове от кегела.
  ///
  /// ⚠⚠ ПРОПОРЦИОНАЛНО, А НЕ „кегел + 2". Шрифтът се мени от бутоните −/+
  /// (13 до 30), а постоянната добавка дава 1,15 от кегела при малкия
  /// шрифт и 1,07 при големия — тоест знакът се смалява спрямо текста,
  /// колкото по-едро чете човек. 1.09 възпроизвежда точно досегашния
  /// размер при подразбиращите се 22 и се държи еднакво по целия обхват.
  /// (Бележка на потребителя, 20.09.2026.)
  ///
  /// ⚠ И ВСИЧКО ОСТАНАЛО ТУК Е В ДЯЛОВЕ ОТ КЕГЕЛА — кутията, повдигането,
  /// отстъпът вдясно. Инак подравняването, измерено при един размер,
  /// се разваля при другите.
  static const double _hramSize = 1.09;

  HtmlExtension get _hramExtension => TagExtension(
        tagsToExtend: const {'hram'},
        builder: (ctx) {
          final f = ReaderFontSize.value;
          final draw = f * _hramSize;
          ...
          return Padding(
            ...
            child: SizedBox(
              ...
              height: f * _hramBox,
              child: OverflowBox(
                // ⚠⚠ ДОЛНИЯТ РЪБ, НЕ СРЕДАТА. По подразбиране `OverflowBox`
                // центрира детето си — а тогава мястото на знака зависи от
                // разликата между кутията и рисунката и се мени с всяка
                // промяна в тях. Закачен за долния ръб, той стъпва на
                // базовата линия като всеки друг глиф.
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, c) {
        ...
        // ⚠ КАРТИНКАТА СЛИЗА С ПОЛОВИН РЕД.
        //
        // Съседният текст е `<p>`, а той носи свой отстъп отгоре (виж
        // `<p>` в reader_styles). Картинката няма такъв, тъй че горният ѝ
        // ръб застава НАД първия ред до нея и тя изглежда „стърчаща".
        // (Бележка на потребителя, 04.09.2026.)
        //
        // ⚠ Половин ред, не цял: целият би я свалил под базовата линия на
        // първия ред и разминаването щеше да е в другата посока.
        final imgTopNudge = ReaderFontSize.value * kReaderLineHeight / 2;
