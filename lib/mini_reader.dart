...
//
// Направен е ОБЩ нарочно: ползвателите му са „Мисли от Теофан Затворник" и
// „От оптинските старци". Затова не знае нищо за съдържанието — получава
// зареждач и съобщение за празен ден.
//
...
class MiniReader extends StatefulWidget {
  ...
  const MiniReader({
    ...
    this.fontSize = 16.5,
    // По-сбито от четеца за цял екран (1.6): тук текстът е кратък и стои в
    // кутийка между другите секции, тъй че широкото междуредие го разлива и
    // го прави да изглежда по-дълъг, отколкото е.
    this.lineHeight = 1.3,
  });
...
class _MiniReaderState extends State<MiniReader> {
  ...
  Map<String, Style> _styles() => {
        ...
        // Редът с името на стареца — „прп. Амвросий". Идва само от
        // optina.db; мислите на свт. Теофан нямат такъв, защото авторът им е
        // един и стои в заглавието на секцията.
        //
        // Вдясно и на един ред от текста, както се подписва цитат. Правилото
        // за <p> отгоре го подравнява двустранно и му дава поле отдолу —
        // тук и двете се отменят.
        '.source': Style(
          fontSize: FontSize(widget.fontSize - 2),
          fontStyle: FontStyle.italic,
          color: AppColors.textSecondary,
          textAlign: TextAlign.right,
          margin: Margins.only(top: 2, bottom: 0),
        ),
