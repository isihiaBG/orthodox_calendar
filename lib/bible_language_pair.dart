...
/// Наредбата при ПРЕГЛЕД НА ЦИТАТ — езикът на цитата вляво, погледът върху него.
///
/// ⚠⚠ ЧИСТА ФУНКЦИЯ нарочно: това е цялото решение „кое отляво, кое отдясно"
/// и трябва да се проверява без екран и без база.
///
///     цитат на друг език  →  ляво: езикът му,  дясно: български
///     цитат на български  →  ляво: български,  дясно: църковнославянски
///
/// И в двата случая `active: 0` — човек вижда цитата веднага, а не след
/// плъзгане. (Поискано от потребителя, 07.09.2026.)
///
/// ⚠ Отсрещната колона е ВИНАГИ вграден превод (`bg` или `utfcs`), тъй че не
/// иска сваляне и не може да остане празна.
///
/// ⚠ Връща `null` при празен език — повреден адрес или стара версия на линка.
/// Тогава нищо не се налага и всичко върви както преди.
BibleLanguagePair? quoteViewPair(String? quoteLang) {
  final lang = quoteLang?.trim() ?? '';
  if (lang.isEmpty) return null;
  return lang == 'bg'
      ? const BibleLanguagePair(first: 'bg', second: 'utfcs', active: 0)
      : BibleLanguagePair(first: lang, second: 'bg', active: 0);
}
