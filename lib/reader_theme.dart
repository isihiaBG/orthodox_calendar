...
class ReaderPalette {
  ...
  /// Заглавието над цитат в режим „цитати" — „Мат. 5:3-12".
  ///
  /// Същото синьо, с което са изписани дяловете в съдържанието на Библията
  /// (`AppColors.sectionTitle`), за да се чете като „заглавие на дял" и тук.
  ///
  /// ⚠ НО САМО в тъмна тема. Върху кремавата страница на светлата същият
  /// цвят дава **2.30:1** — под всяка норма, тоест надписът просто избледнява.
  /// Затова светлата стойност е своя, по-наситена (4.5:1 при получер и
  /// по-едър кегел, което е над искането на WCAG за едър текст).
  ///
  /// ⚠ Това е ТРЕТИЯТ случай в проекта, в който закован цвят от [AppColors]
  /// се оказва верен само в едната тема — виж правилото при [sheet].
  Color get heading =>
      dark ? const Color(0xFF8A9BB0) : const Color(0xFF4E6A90);

  /// Фонът зад стиховете, посочени от ПРЕПРАТКА — цитатът, заради който
  /// човек е дошъл тук от някое житие.
  ///
  /// ⚠ ТРЕТИ смисъл, затова и трети цвят. Жълтото значи „намерено при
  /// търсене", [here] значи „ти си тук" в съдържанието, а това значи „ето
  /// точно каквото поиска". Слеят ли се двата сини, човек престава да чете
  /// цвета като указание.
  ///
  /// Подбрано с ИЗМЕРВАНЕ, не на око, защото е фон под ДЪЛЪГ текст и грешката
  /// тук се плаща при всеки ред:
  ///   • спрямо страницата — 1.79x в тъмна, 1.78x в светла, тоест еднакво
  ///     силно личи и в двете (несъразмерността се вижда веднага при
  ///     превключване на темата);
  ///   • текстът върху него — 8.0:1 и 7.9:1, тоест над AAA (7:1). Обикновено
  ///     4.5:1 стига, но тук се чете цяло Писание, а не етикет.
  ///
  /// ⚠ В светла тема числото лъже НАДОЛУ: страницата е топло кремава, а това
  /// е студено синьо, тъй че видяната разлика е по-голяма от смятаната.
  /// Затова светлата стойност е по-мека, отколкото сметката допуска.
  Color get quote => dark ? const Color(0xFF2D4158) : const Color(0xFF9DB1C8);
