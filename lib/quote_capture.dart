...
int _pick(List<int> all, String want, FoldedBlocks f, (int, int)? hint) {
  if (hint != null) {
    // ⚠⚠ БЛОКЪТ ТЕЖИ ПОВЕЧЕ ОТ ЗНАКА — те идват от различни по надеждност
    // неща. Блокът е от истински хит-тест върху геометрията (`_hintFor` в
    // quote_menu.dart); знакът е ГРУБА ОЦЕНКА — дял от височината на кутията,
    // умножен по дължината на текста.
    //
    // При ИЛЮСТРАЦИОНЕН регион тази оценка е систематично грешна: надписът
    // стои в НАЧАЛОТО на текста (знак 0, защото `regionPlainTexts` го слага
    // пръв), а в КУТИЯТА е най-отдолу — под самата картинка. Тъй че маркиран
    // надпис дава дял ~0,9 и оценката сочи края на региона.
    //
    // Докладвано от потребителя на 06.09.2026: житието на св. Кирил Философ
    // има ДВА съседни надписа с еднакъв първи ред. Регион 319 е само надпис
    // (129 знака), регион 320 е надпис плюс 849 знака обтичащ текст; двете
    // съвпадения са на 101 сгънати знака едно от друго. Маркираният беше
    // първият, оценката падаше на знак ~116 от 129 и най-близкото излизаше
    // второто — на 9 знака срещу 110.
    //
    // Затова: има ли съвпадение В ПОСОЧЕНИЯ БЛОК, печели то. Знакът решава
    // само измежду няколко съвпадения в един и същ блок, където оценката е
    // достатъчно добра, а грешката ѝ — ограничена от дължината му.
    final h = f.foldedIndexOf(hint.$1, hint.$2);
    final inHinted = all.where((a) => f.ofBlock[a] == hint.$1).toList();
    final pool = inHinted.isNotEmpty ? inHinted : all;
    var best = pool.first;
    for (final a in pool) {
