...
/// ⚠⚠ ПАЗЯТ СЕ БУКВИ И ЦИФРИ ОТ ВСЯКА ПИСМЕНОСТ, не изброен обхват.
///
/// Дотук стоеше `[0-9a-zа-яёіѣѫ]` — латиница, кирилица и шепа славянски
/// добавки. Оттам БЪГ, който правеше цитатите НЕВЪЗМОЖНИ за десет от
/// дванайсетте превода: маркираш дума на гръцки, сгънатото излиза ПРАЗНО,
/// `captureSelection` честно се отказва и на екрана пише „Маркирай откъс в
/// рамките на един абзац" — дори при една-единствена дума.
/// (Докладвано от потребителя, 07.09.2026.)
///
/// Измерено преди поправката:
///
///     български „Свети Иоан Рилски"     → 15 знака
///     цс        „Єгда же прииде петръ"  → 24 знака (но без ѡ, ѧ, ꙋ)
///     гръцки    „Ἐν ἀρχῇ ἦν ὁ λόγος"     → 0  ⚠
///     иврит     „בְּרֵאשִׁית בָּרָא"            → 0  ⚠
///     грузински „იყო სიტყუაჲ"            → 0  ⚠
///
/// ⚠ `\p{M}` (комбиниращите знаци) НЕ влиза нарочно: сгъването трябва да
/// остане нечувствително към ударения и надредни знаци, а точно те се менят
/// при редакция. Проверено: „́" отпада, „ꙋ" остава.
///
/// ⚠⚠ СЛЕДСТВИЕ ЗА ВЕЧЕ СПОДЕЛЕНИТЕ ЛИНКОВЕ. За български и латински
/// сгъването е БУКВАЛНО непроменено, тъй че техните отпечатъци си съвпадат.
/// Църковнославянските се променят (ѡ, ѧ, ꙋ вече влизат) — но отпечатък,
/// който не съвпада, просто пада на ГОЛИТЕ КООРДИНАТИ и цитатът пак се
/// отваря на мястото си. Тоест влошаването е плавно, а печалбата е, че десет
/// превода изобщо проработват.
String foldForMatch(String s) {
  final b = StringBuffer();
  for (final r in s.toLowerCase().runes) {
    final c = String.fromCharCode(r);
    if (kWordChar.hasMatch(c)) b.write(c);
  }
  return b.toString();
}

/// Кои знаци оцеляват при сгъването — буква или цифра от ВСЯКА писменост.
///
/// ⚠⚠ ЕДНО ОПРЕДЕЛЕНИЕ ЗА ДВЕТЕ СГЪВАНИЯ. [foldForMatch] сгъва ТЪРСЕНОТО, а
/// [foldBlocks] — ЧЕТИВОТО, и двете трябва да пазят едно и също. Дотук бяха
/// два отделни израза и поправката в единия остави другия със стария обхват:
/// сгънатият избор се съдържаше в сгънатия текст, а улавянето пак се
/// отказваше. Разминаване, което личи само на екрана и не гърми никъде.
///
/// ⚠ Строи се ВЕДНЪЖ. Дотук изразът се създаваше НАНОВО за всеки знак, а
/// сгъването обхожда цели четива — при 107 000 знака това са 107 000
/// компилации на един и същи регекс.
final RegExp kWordChar = RegExp(r'[\p{L}\p{N}]', unicode: true);
