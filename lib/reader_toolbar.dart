...
/// Височината на лентата с инструменти (AppBar/SliverAppBar) в четците.
/// ОБЩА за reader_screen.dart и book_reader.dart, и за двете им подредби
/// (заглавна лента отгоре при четене, sliver при него) — вижте
/// kSearchBarHeight за спътника ѝ.
///
/// ⚠ Ползва се и в сметката за корекция при отваряне/затваряне на
/// търсенето (reader_screen._kSearchChromeHeight,
/// book_reader._kSearchChromeHeight) — точно затова е тук като кръстена
/// константа, а не гола 44 на всяко от петте места, където двата четеца я
/// ползват: смени ли се височината утре, корекцията се сменя автоматично
/// с нея, вместо тихо да се разкачи.
const double kReaderToolbarHeight = 44.0;

/// Височината на лентата за търсене (полето за писане + бутоните до нея)
/// в четците — вижте бележката при [kReaderToolbarHeight].
const double kSearchBarHeight = 58.0;
...
/// Копчето „Търсене" — за екрани ИЗВЪН четците, които искат същата лупа.
///
/// ⚠ Стои тук, а не се преписва на място. В указателя на Библията отначало
/// беше сложено `RoundIconButton` — то рисува кръгче около иконката, каквото
/// лупата в трите четеца НЯМА (кръгът там се появява само когато търсенето е
/// отворено). Разликата е дребна поотделно и очевидна в движение: едно и
/// също действие изглежда по два начина в едно приложение.
Widget readerSearchButton({
  required BuildContext context,
  required VoidCallback onTap,
  bool active = false,
}) {
