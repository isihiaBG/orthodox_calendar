...
const int kQuoteLinkVersion = 3;

/// ⚠ ВЕРСИЯ 3 (05.09.2026) не добавя нищо — само СВИВА. Адресът падна от 156
/// на около 70 знака, по искане на потребителя („тези дълги линкове не стоят
/// никак естетично"). Три промени, всяка измерена:
///
///   1. **Двоичен пакет вместо текстов.** Дотук числата стояха като текст с
///      разделители („49|493|6|"), а отпечатъкът — като осем шестнайсетични
///      знака вместо четири байта. Сега всичко е varint и сурови байтове.
///   2. **Краят и текстът отпадат, когато не носят нищо.** Краят се извежда
///      от началото и дължината при цитат в един блок (обичайното), а самият
///      откъс вече изобщо не пътува: целият цитат и без това стои в
///      съобщението, над линка.
///   3. **Локаторът се свива.** За житие — четирибайтов отпечатък на слъга
///      (28 знака стават 4); за книга — номер на тома и на главата, вместо
///      цялото име на файла. ⚠ И двете имат ЛИТЕРАЛЕН резервен път: не се ли
///      свие еднозначно, слъгът влиза цял.
///
/// ⚠ Версии 1 и 2 продължават да се четат. Разпознават се по ПЪРВИЯ знак на
/// пакета: „r"/„z" са текстовите, „b"/„c" — двоичният.
///
...
class QuoteAnchor {
  ...
  Map<String, dynamic> toJson() => {
        ...
        'c': charStart,
        'n': charLength,
        // ⚠ Пише се само НЕизводимото — така старите записи се четат без
        // промяна и файлът не расте без нужда.
        if (blockEnd != block) 'be': blockEnd,
        // ⚠⚠ УСЛОВИЕТО ЗА КРАЯ Е СВОЕ, а НЕ „през няколко абзаца".
        //
        // Дотук и двете стояха на `blockEnd != block`, защото при обикновено
        // четиво краят наистина се извежда като `charStart + charLength`. За
        // ПИСАНИЕТО обаче числата значат друго (виж докстринга на
        // [QuoteAnchor]): charStart е отрязано ОТПРЕД, charEnd — ОТЗАД, а
        // charLength изобщо не се ползва. Тъй че цитат в ЕДИН стих не си
        // записваше отрязването отзад, при четене то се извеждаше равно на
        // charStart, диапазонът излизаше празен и маркирането падаше на
        // резервния път: ЦЕЛИЯТ стих.
        //
        // (Докладвано от потребителя на 06.09.2026 — споделеният линк за
        // същия откъс работеше, защото `_packV3` открай време ползва точно
        // условието отдолу.)
        if (charEnd != charStart + charLength) 'ce': charEnd,
...
class QuotesStore {
  ...
  static Future<List<Quote>> load() async {
    ...
    // ⚠⚠ ИЗМЕНИМ списък, НЕ `const []`.
    //
    // Тук стоеше `const []` и оттам идваше бъг, който изглеждаше съвсем
    // другояче: при ПЪРВО влизане в „Любими цитати" (преди да е записано
    // каквото и да е) екранът въртеше спинера безкрайно.
    //
    // Веригата: ключът го няма → връща се НЕИЗМЕНИМ списък → повикващият
    // прави `quotes.sort(...)` → „Cannot sort immutable List" → грешката се
    // губи като необработена асинхронна и `_items` остава `null`, тоест
    // спинер. Запишеш ли един цитат, `jsonDecode` дава истински списък и
    // всичко тръгва — затова се проявяваше САМО първия път.
    // (Докладвано от потребителя, 06.09.2026.)
    //
    // ⚠ Правилото е общо: метод, чийто резултат ЩЕ СЕ МЕНИ от повикващия, не
    // бива да връща `const []`. Проверено — `BibleScopePresets.sorted()`
    // копира преди да сортира и там капанът го няма.
