  ...
  for (final m in RegExp(r'<[^>]*>|[^<]+').allMatches(html)) {
    ...
    if (piece.startsWith('<')) {
      ...
      if (t.startsWith('<strong') || t.startsWith('<b>')) {
        ...
        push('\n');
        lastWasSpace = true;
      } else if (t.startsWith('<hram')) {
        // ⚠⚠ ЦЪРКВИЦАТА ПРЕД ЦЪРКОВНАТА ДАТА СТАВА ДУМА ТУК — И ТОВА Е
        // НАРОЧНО.
        //
        // Живата дата (church_dates.dart) бележи църковната половина с
        // `<hram></hram>`, а четецът го рисува с `Icons.church`. Но ТОЗИ
        // абзац не минава през flutter_html: регионът с буквицата се
        // рисува на ръка с `Text.rich` от тези парчета. Непознат таг се
        // сваля мълчаливо — тоест знакът щеше просто да изчезне точно в
        // началото на четивото, а там пада всяка втора цитирана дата
        // (мерено: 53 от 113 четива).
        //
        // ⚠ Иконка тук значи WidgetSpan, а мерещият TextPainter не може да
        // оформи WidgetSpan без подадени размери на запълнителите — виж
        // `forMeasure` в spansOf. Тогава мерещият и рисуващият трябва да
        // дават ЕДНАКВА ширина, а ширината на иконка не се възпроизвежда с
        // текст. Правилото на този файл („мерещият е огледален на
        // рисуващия") е по-важно от еднаквия облик.
        //
        // ⚠ Същата дума стои и в PDF-а (`_parseBlocks` в pdf_export.dart),
        // тъй че видовете са два, а не три.
        push('църк. ');
        lastWasSpace = false;
      }
      continue;
