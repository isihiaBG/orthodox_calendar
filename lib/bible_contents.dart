      ...
      children: [
        ...
        if (extra.isNotEmpty)
          _kathismaSection(
            book,
            key: 'extra',
            // ⚠ „Допълнение" е СЪЩЕСТВИТЕЛНО и НЕ се съгласува по число
            // (поискано от потребителя, 06.09.2026). Дялът е един —
            // допълнението към Псалтира, — колкото и псалма да съдържа: и
            // при един, и при няколко се казва „Допълнение", а не
            // „Допълнения". Затова тук няма условие, за разлика от
            // `trailing` до него, който брои самите псалми.
            //
            // Досега стоеше прилагателно („Допълнителен"), което се
            // съгласуваше с подразбиращия се „псалм" — оттам и нуждата от
            // двете форми.
            title: 'Допълнение',
            trailing: extra.length == 1 ? 'Пс. ${extra.first}' : '${extra.length}',
            numbers: extra,
            // ⚠ ЕДИНСТВЕНАТА линия в този таб. Двайсетте катизми са един
            // непрекъснат ред и черта помежду им не казва нищо; „Допълнение"
  ...
  Widget _kathismaSection(
    ...
    return Column(
      ...
      children: [
        ...
        // редът „Допълнение" — той сам е заглавието на своя дял.
        if (dividerAbove) ...[
        ...
        // таб стои НАД „Допълнение" (`dividerAbove`).
      ],
    );
  }
