...
class IncomingQuoteLinks {
  ...
  static Future<void> start({
    ...
    Future<void> handle(Uri uri, {required bool appIsUp}) async {
      ...
      if (parsed == null) {
        // ⚠⚠ ДВА РАЗЛИЧНИ СЛУЧАЯ, и мълчанието е вярно САМО за единия.
        //
        // Чужд адрес наистина не е наша работа — мълчи се. Но адрес ПОД
        // нашия път (`/q/…`) е предназначен за нас и щом не се разчита,
        // той е ПОВРЕДЕН: такива се получават, когато линкът е сглобен на
        // ръка. Тогава мълчанието е най-лошият изход — приложението се
        // отваря в календара и човек няма представа защо.
        // (Поискано от потребителя, 11.09.2026.)
        if (uri.path.startsWith(kQuotePath)) {
          final ctx = navigatorKey.currentContext;
          if (ctx != null && ctx.mounted) {
            ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(
              content: Text('Този адрес не може да се разчете.'),
            ));
          }
        }
        return;
