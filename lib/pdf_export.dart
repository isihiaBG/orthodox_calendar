...
List<_Block> _reorderBlocks(
  ...
  // ⚠⚠ ТРОПАРИТЕ И КОНДАЦИТЕ СА КРАЯТ, НЕ ПОВЕСТВОВАНИЕТО.
  //
  // Правило на потребителя (05.09.2026): илюстрацията се мести В РАМКИТЕ НА
  // ТЕКСТА и е по-добре да не отива в тропарите и кондаците — те са нещо
  // като подвал на разказа.
  //
  // Дотук мениджърът нямаше никаква представа за тях: имаше забрани да
  // прескача КАРТИНКА, ЗАГЛАВИЕ и реда с ИЗТОЧНИКА, но песнопенията му бяха
  // обикновен текст, тъй че бутането напред минаваше през тях. Отвън това
  // изглежда особено зле, защото зоната после обтича текста на тропара.
  //
  // ⚠ Границата се търси по ПЪРВОТО песнопение, а не по всяко: щом веднъж
  // започнат, нататък до края са само те и редът с източника.
  var firstPrayer = unit.length;
  for (var i = 0; i < unit.length; i++) {
    final c = blocks[unit[i].first].cls;
    if (c.contains('prayerhead') || c.contains('csl') || c.contains('trans')) {
      firstPrayer = i;
      break;
    }
  }
  ...
  for (var pos = 0; pos < order.length; pos++) {
    ...
    for (var k = 1; k <= _kImageShiftLimit && pos + k < order.length; k++) {
      ...
      if (uImg[nxt] || blocks[unit[nxt].first].cls.contains('source')) break;
      // ⚠ И не се влиза в песнопенията — виж [firstPrayer].
      if (pos + k >= firstPrayer) break;
      probe = uSplit[nxt]
...
int _addFlowImage({
  ...
  // ⚠⚠ КАРТИНКАТА СЛИЗА С ПОЛОВИН РЕД — същото, което вече правим в четеца
  // (`imgTopNudge` в reader_screen.dart).
  //
  // Съседният абзац носи СВОЙ отстъп отгоре, а картинката няма: без този
  // отстъп горният ѝ ръб стърчи над първия ред от обтичащия текст и окото
  // веднага усеща разминаването. (Докладвано от потребителя за PDF-а,
  // 05.09.2026: „същият проблем преди време поправихме и в хоризонталния
  // изглед в четеца, а сега се е появил и при PDF-а — или по-скоро си е бил
  // тук, но не сме му обръщали внимание".)
  //
  // ⚠⚠ СТОЙНОСТТА Е ИЗМЕРЕНА, НЕ ИЗВЕДЕНА — по установената в проекта
  // практика за вертикалното подравняване в pdf пакета (виж „ДВА КАПАНА В
  // pdf ПАКЕТА" в CLAUDE.md: там теоретичните сметки сгрешиха два пъти
  // подред, а опитът уцели веднага).
  //
  // Мерено върху нарисувана страница (`pdftoppm -r 72 -gray`, сравнен горен
  // ръб на снимката с горния ред текст до нея):
  //
  //     без отстъп            картинката е ~10 pt НАД текста  ← „стърчи"
  //     половин РЕД (16,2)    картинката е  +6 pt ПОД текста
  //     половин КЕГЕЛ (10)    подравнени
  //
  // ⚠ Не е половин РЕД, а половин КЕГЕЛ. В четеца формулата е другата
  // (`ReaderFontSize.value * kReaderLineHeight / 2`), защото там отстъпите
  // на абзаца идват от flutter_html и геометрията е различна. Не ги
  // изравнявай „за последователност" — мери.
  final imgTopNudge = bodySize / 2;

  // ⚠ Отстъпът влиза и в ЗОНАТА: колоната вдясно има толкова повече място,
  // а не се ли добави, текстът се реже по-рано и под него зее.
  var zoneHeight = imgTopNudge + size.$2;
  if (capBlock != null) {
  ...
  if (used > zoneHeight && beside.isNotEmpty) {
    ...
    // ⚠⚠ СТЪПКАТА НА РЕДА Е [_targetLineEm], НЕ [_lineHeight].
    //
    // Тук стоеше `_lineHeight` (1.45) — вътрешната мярка на pdf пакета, а не
    // онова, което реално се получава на листа. Действителната стъпка е 1.62
    // em (мерено в готов PDF: 32,40 pt при кегел 20 — виж [_lineSpacing],
    // която нарочно я нагласява точно там).
    //
    // Разликата е 12% и води до завишен `linesFit`, тоест разрезът оставя
    // ПОВЕЧЕ редове, отколкото се побират: измерено, текстовата колона
    // излизаше 624 pt при зона 559, а цялата зона стигаше до 837 pt при
    // 762 използваема височина на листа. Оттам и празните страници —
    // `Inseparable`, по-висок от листа, няма къде да се сложи.
    final lineH = fs * _targetLineEm;
  ...
  final block = pw.SizedBox(
    ...
    child: pw.Column(
      ...
      mainAxisSize: pw.MainAxisSize.min,
      children: [
        pw.SizedBox(height: imgTopNudge),
