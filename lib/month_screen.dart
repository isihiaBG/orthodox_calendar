  ...
  Future<void> _loadMonth({bool preserveScroll = false}) async {
    ...
    while (!current.isAfter(rangeEnd)) {
      ...
      final saints = await db.rawQuery('''
        ...
        WHERE s.date = ?
        ORDER BY
        -- ⚠⚠ КОМЕНТАРИТЕ (ранг 0) ИЗЛИЗАТ ПРЕДИ ВСИЧКО ОСТАНАЛО, независимо от
        -- поместната църква. Иначе „Съединение на България" и „Ден на народните
        -- будители" — BG, ранг 0 — падаха НАЙ-ОТДОЛУ, след целия вселенски
        -- списък, защото подредбата тръгва от групата.
        -- (Искане на потребителя, 06.09.2026.)
        --
        -- ⚠ Те не са памет на светия, а бележка за деня: мястото им е най-горе,
        -- както е и при вселенските коментари („Навечерие на Богоявление",
        -- „Начало на Великия пост"), които и досега излизаха там — но само по
        -- случайност, защото ECUMENICAL е първата група.
        --
        -- ⚠ `CASE`, а не `s.rank <> 0`: при NULL ранг вторият дава NULL, а NULL
        -- в SQLite се подрежда ПРЕДИ всичко — тъй че запис без ранг би скочил
        -- пред самите коментари. Днес такива няма, но изразът не бива да зависи
        -- от това.
        CASE WHEN s.rank = 0 THEN 0 ELSE 1 END ASC,
            sg.id ASC, s.rank ASC
        LIMIT 3
