...
void quoteVsSearchTests() {
  ...
  // ── Сгъването и чуждите писмености ───────────────────────────────────────
  //
  // ⚠⚠ Докладвано на 11.09.2026: цитат на гръцки не можеше нито да се запази,
  // нито да се сподели — „Маркирай откъс в рамките на един абзац" дори при
  // една дума. `foldForMatch` пазеше само [0-9a-zа-яёіѣѫ], тъй че гръцки,
  // иврит и грузински се свиваха до НУЛА знака.
  group('сгъването пази всяка писменост', () {
    test('⚠ гръцки, иврит и грузински вече НЕ изчезват', () {
      expect(foldForMatch('Ἐν ἀρχῇ ἦν ὁ λόγος'), isNotEmpty, reason: 'гръцки');
      expect(foldForMatch('בְּרֵאשִׁית בָּרָא'), isNotEmpty, reason: 'иврит');
      expect(foldForMatch('იყო სიტყუაჲ'), isNotEmpty, reason: 'грузински');
    });

    // ⚠⚠ НАЙ-ВАЖНОТО: вече споделените линкове не бива да се счупят. За
    // български и латиница сгъването трябва да е БУКВАЛНО непроменено.
    test('⚠ български и латиница са НЕПРОМЕНЕНИ', () {
      expect(foldForMatch('Свети Иоан Рилски'), 'светииоанрилски');
      expect(foldForMatch('In the beginning was the Word'),
          'inthebeginningwastheword');
      expect(foldForMatch('Числа 1, 2 и 3!'), 'числа12и3');
    });

    test('пунктуацията и интервалите пак отпадат', () {
      expect(foldForMatch('  а, б — в!  '), 'абв');
      expect(foldForMatch('«цитат»'), 'цитат');
    });

    // ⚠ Комбиниращите знаци НЕ влизат: сгъването трябва да остане
    // нечувствително към ударения, защото точно те се менят при редакция.
    test('⚠ ударенията и надредните знаци отпадат', () {
      expect(foldForMatch('Бра́тие'), foldForMatch('Братие'));
      expect(foldForMatch('Тео́фил'), foldForMatch('Теофил'));
    });

    test('цитат на гръцки се улавя и намира наново', () {
      const blocks = ['Ἐν ἀρχῇ ἦν ὁ λόγος, καὶ ὁ λόγος ἦν πρὸς τὸν θεόν.'];
      final spot = captureSelection(blocks, 'ὁ λόγος ἦν πρὸς τὸν θεόν');
      expect(spot, isNotNull, reason: 'улавянето вече не се отказва');
      final q = buildQuote(
        source: QuoteSource.bible,
        locator: 'g|Jn|1',
        title: 'Йоан. 1:1',
      ...
      expect(q.text, 'ὁ λόγος ἦν πρὸς τὸν θεόν');
    });
  });
}
