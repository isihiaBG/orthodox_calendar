...
def _fixed_day_detail(date: datetime.date, year: int, old_style: bool,
    ...
    # ⚠⚠ ОТСИЧАНЕТО НА ГЛАВАТА НА СВ. ЙОАН ПРЕДТЕЧА (29.VIII църковно) — ВИНАГИ
    # олио, независимо от деня на седмицата.
    #
    # Без това правило денят падаше в общата каскада, а там светия от РАНГ 2
    # („Бдение") в сряда или петък дава РИБА — и точно това се случваше:
    #
    #     нов стил   29.08.2025 (петък)  → риба ⚠
    #     стар стил  11.09.2026 (петък)  → риба ⚠
    #
    # Риба на този ден е недопустима: това е еднодневен пост в памет на
    # мъченическата смърт на Предтечата, а не празнично разрешение.
    # (Докладвано от потребителя, 11.09.2026.)
    #
    # ⚠ Стои ТУК, до Въздвижение, по същия довод: и двата дни са изключения
    # от общото правило „голям празник → облекчение" и трябва да се хванат
    # ПРЕДИ него. Денят вече е в `single_day` за `fast_period`, но то решава
    # само ЧЕ е постен, не КАКЪВ е постът.
    if civil_from_church(year, 8, 29, old_style) == date:
        return (4, 'beheading_forerunner')
