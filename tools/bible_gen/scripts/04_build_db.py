...
def main():
    ...
    # ⚠ И ВСЯКО МЯСТО СЪС СТИХ „0" — независимо кой превод го бележи.
    #
    # Горното събиране минава ВЪТРЕ в цикъла по ИЗБРАНИТЕ езици, а източникът
    # бележи надписанието надеждно само в руския и в църковнославянския. Щом
    # руският излезе от основната база (тя носи само `bg` и `utfcs`), с него си
    # отиват и надписанията, които само той бележи: 26 места, между тях Пс. 1,
    # 2, 10, 13 и 14. Отвън това личи като „«Псалом Давидов» не е в курсив" —
    # текстът си е там, но никой не знае, че е надписание.
    #
    # Правилото тук не зависи от езика и затова не може да се загуби:
    # НОМЕРАЦИЯТА НА СТИХОВЕТЕ ЗАПОЧВА ОТ 1, тъй че непразен стих „0" е
    # надписание по определение.
    #
    # ⚠ Проверява се за НЕПРАЗЕН текст. Църковнославянският носи стих „0" и
    # там, където няма надписание (в Пс. 1 например той е празен) — вписан,
    # той би дал курсив на празно място.
    con.execute("""
        INSERT OR IGNORE INTO headings (book, chapter, verse)
        SELECT DISTINCT book, chapter, verse FROM verses
        WHERE verse = '0' AND trim(coalesce(text, '')) <> ''
    """)
