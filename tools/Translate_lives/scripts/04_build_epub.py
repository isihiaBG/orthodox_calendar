...
def https_links(s):
    """Всички адреси към azbyka.ru — по СИГУРНА връзка. Връща (текст, брой).

    Книгите носят част от препратките по `http://`; сайтът отдавна е минал на
    https и ги пренасочва, но това е по един излишен прескок на препратка, а
    по мобилна мрежа се усеща. Замяната е обща, не само за Писанието: в
    томовете има и препратки към „Отечник".
    """
    return re.subn(r'http://(azbyka\.ru)', r'https://\1', s)
...
# Препратка към бележка вътре в съдържанието: номерът стои И в href-а
# (`#note1088`), И видимо в горния индекс. И двете трябва да тръгнат от
# едно и също старо число — assert-ът по-долу пази точно това.
RE_NOTE_REF = re.compile(
    r'(<a\s+href="[^"]*#note)(\d+)("[^>]*>\s*<sup[^>]*>)(\d+)(</sup>\s*</a>)')

# Самата бележка в notes.xhtml: номерът стои в `id="note1088"` И като гол
# текст малко по-долу, точно преди <br/>. Виж примера в докстринга на
# renumber_notes().
RE_NOTE_DEF = re.compile(
    r'(<h1[^>]*\bid="note)(\d+)("[^>]*>)(.*?)(\n\s*)(\d+)(\s*<br)', re.S)


def renumber_notes(oebps):
    """Всеки том да брои бележките си от 1, не от там, където е спрял
    предишният.

    Оригиналът е една книга в 12 печатни тома с ЕДНА обща номерация на
    бележките през целия комплект — януари 1–1087, февруари 1088–1678,
    март 1679–2334 и т.н., без прекъсване (проверено за всичките 12).
    За печатна книга това е нормално; за отделни .epub файлове е объркващо
    — читателят вижда бележка № 1501 да е първата в тома.

    ⚠ Пипа се ТУК, при сглобяването — НЕ в src/ и камо ли в оригиналните
    .epub-и на Input/. Те се пресъздават от 01_merge_notes.py и всяка ръчна
    поправка там би изчезнала при следващото пускане. Разковничето е, че
    номерът никъде не носи смисъл отвъд себе си (просто пореден низ вътре в
    `id`/`href`), тъй че отместването е чисто козметично и не иска пипане
    на превода, на `manual_notes.json` (той сочи бележки по ОРИГИНАЛНИЯ им
    номер, приложен преди тази стъпка) или на самите файлове с бележки.

    Как. Номерът на бележка се среща на ДВЕ независими места, всяко от
    които трябва да остане вътрешно съгласувано:

        препратка (Text/*.xhtml):
            <a href="../Text/notes.xhtml#note1501" title="…">
              <sup class="calibre8">1501</sup></a>

        самата бележка (Text/notes.xhtml):
            <h1 id="note1501" class="calibre9">…
            1501 <br class="calibre10"/></h1>

    Затова стъпките са ТРИ, в този ред:
      1. видимият номер в <sup> се пренаписва по числото от href-а до него
         (href-ът засега остава непипнат — той е СВОЙ извор на истина);
      2. видимият гол номер в notes.xhtml се пренаписва по числото от
         съседния `id="noteN"` (по същата причина — id остава непипнат);
      3. чак СЕГА `id="noteN"`/`#noteN` навсякъде се измества с общото
         число — проверено емпирично (17.08.2026), че низът „note" следван
         от цифри се среща В ЦЕЛИЯ том САМО в тези две роли (591 id срещу
         591 href в проверения том, нито едно другаде), тъй че простото
         търсене на `note\\d+` е безопасно и не пипа странична бройка,
         дата или друго число в текста.

    Връща (старо_начало, старо_край, изместване) или None, ако томът няма
    бележки (notes.xhtml липсва).
    """
    notes_path = os.path.join(oebps, "Text", "notes.xhtml")
    if not os.path.exists(notes_path):
        return None

    номера = [int(n) for n in
              re.findall(r'id="note(\d+)"', open(notes_path, encoding="utf-8").read())]
    if not номера:
        return None
    старо_начало, старо_край = min(номера), max(номера)
    изместване = старо_начало - 1
    if изместване == 0:
        return (старо_начало, старо_край, 0)   # януари — вече от 1, нищо за пипане

    def нов_sup(m):
        old_href, old_sup = int(m.group(2)), int(m.group(4))
        assert old_href == old_sup, (
            "href/sup разминаване при note%d/note%d — структурата на "
            "препратката се е променила, провери на ръка" % (old_href, old_sup))
        n = old_href - изместване
        return "%s%d%s%d%s" % (m.group(1), old_href, m.group(3), n, m.group(5))

    def нов_def(m):
        old_id, old_txt = int(m.group(2)), int(m.group(6))
        assert old_id == old_txt, (
            "id/текст разминаване при note%d/%d в notes.xhtml — провери на ръка"
            % (old_id, old_txt))
        n = old_id - изместване
        return "%s%d%s%s%s%d%s" % (
            m.group(1), old_id, m.group(3), m.group(4), m.group(5), n, m.group(7))

    for path in glob.glob(os.path.join(oebps, "Text", "*.xhtml")):
        s = open(path, encoding="utf-8").read()
        if "#note" not in s and 'id="note' not in s:
            continue
        нов = RE_NOTE_REF.sub(нов_sup, s)
        нов = RE_NOTE_DEF.sub(нов_def, нов)
        # чак сега — общото изместване на id/href навсякъде в тома
        нов = re.sub(r"\bnote(\d+)\b",
                     lambda m: "note%d" % (int(m.group(1)) - изместване), нов)
        if нов != s:
            with open(path, "w", encoding="utf-8") as f:
                f.write(нов)

    return (старо_начало, старо_край, изместване)
...
def process(vol, dry, epub3=False):
    ...
    # ⚠ ЧАК ТУК, СЛЕД цикъла с превода — не преди него. Видимият номер на
    # бележка не е структура (href/id), а е ТЕКСТ на блока, вкаменен в
    # JSON-а от превода: `split_tags()` разглобява по ЕДИНИЧЕН таг
    # (`RE_TAG`), тъй че цифрите между <sup> и </sup> остават като гола
    # текстова буква в блока, който `rebuild_file()` после залепва обратно
    # от `tr`. Пуснато преди цикъла, преномерирането оправя href-а, но
    # rebuild_file() веднага презаписва видимия номер със стария от
    # превода — открито едва след като потребител отвори готовия .epub и
    # видя „Апамея¹⁰⁸⁸" с работещ линк към вярната note1.xhtml (17.08.2026).
    бр = renumber_notes(oebps)
    if бр and бр[2]:
        print("  бележки: %d–%d → 1–%d (том по том, не през целия комплект)"
              % (бр[0], бр[1], бр[1] - бр[2]))
    elif бр:
        print("  бележки: 1–%d (вече от 1 — първи том)" % бр[1])
