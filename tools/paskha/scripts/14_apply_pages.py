...
# Етикетът се маха от самия текст и се слага като `<span>` — както при
# песнопенията (`_prayersBlocksHtml`), за да е приглушен, а преводът курсив.
RE_ЕТИКЕТ = re.compile(r'^\s*(?:Превод|Перевод)\s*:\s*')
    ...
    for i, s in enumerate(d['sections']):
        ...
        for j, (текст, блок) in enumerate(zip(п['blocks_bg'], s['blocks'])):
            if литургичен:
                ...
                # ⚠⚠ СЪЩИТЕ КЛАСОВЕ КАТО ПРИ ПЕСНОПЕНИЯТА (`.csl` / `.trans`
                # + `.translabel`). Богослужебният текст и преводът под него
                # са едно и също нещо тук и в „Тропар и кондак", тъй че
                # трябва и да изглеждат еднакво: етикетът приглушен,
                # преводът курсив. (Искане на потребителя, 20.09.2026.)
                if RE_ПРЕВОД.match(ru):
                    гол = RE_ЕТИКЕТ.sub('', текст)
                    части.append(
                        '<p class="trans"><span class="translabel">Превод:'
                        '</span> %s</p>'
                        % връзки(гол, блок.get('links') or [], налични))
                else:
                    # ⚠ Руският оригинал НЯМА запушалки за връзки (сверено:
                    # в този дял връзки изобщо няма), тъй че подмяната не
                    # губи нищо.
                    части.append('<p class="csl">%s</p>'
                                 % връзки(ru, блок.get('links') or [], налични))
                continue
