...
# ⚠⚠ БОГОСЛУЖЕБНИЯТ ТЕКСТ НЕ СЕ ПРЕВЕЖДА — той Е църковнославянски, само
# изписан с руска гражданска азбука (същото, което прави и колоната `csl`
# при песнопенията). Преведен, той престава да е онова, което се чете на
# служба. Затова в изброените дялове остава РУСКИЯТ оригинал, а на
# български минават САМО редовете, които сами се обявяват за превод.
# (Решение на потребителя, 20.09.2026.)
#
# ⚠ Слъг → номера на ДЯЛОВЕ. Уводът на статията е обикновена проза (и
# носи всичките връзки), тъй че той се превежда както обикновено.
БОГОСЛУЖЕБНИ = {
    'chasy-pasxalnye': {1},          # „Текст Часов Пасхи"
}

# „Перевод:" остава; „Перевод 2." отпада ИЗЦЯЛО — втори вариант на същия
# превод само удвоява четивото.
RE_ПРЕВОД = re.compile(r'^\s*Перевод\s*:')
RE_ПРЕВОД2 = re.compile(r'^\s*Перевод\s*2')
...
def сглоби(слъг: str, налични: set, картинки: dict) -> dict | None:
    ...
    for i, s in enumerate(d['sections']):
        ...
        литургичен = i in БОГОСЛУЖЕБНИ.get(слъг, ())
        for j, (текст, блок) in enumerate(zip(п['blocks_bg'], s['blocks'])):
            if литургичен:
                ru = блок['ru']
                if RE_ПРЕВОД2.match(ru):
                    continue             # вторият превод отпада изцяло
                if not RE_ПРЕВОД.match(ru):
                    # ⚠ Руският оригинал НЯМА запушалки за връзки (сверено:
                    # в този дял връзки изобщо няма), тъй че подмяната не
                    # губи нищо.
                    текст = ru
