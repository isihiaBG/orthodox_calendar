...
def _обходи_пътека(възел, пътека, преобрази, промени, адрес=''):
    ...
    if ключ == '*':
        # ⚠⚠ „*" значи ВСИЧКИ ЕЛЕМЕНТИ — и на списък, И на речник. Дотук
        # поемаше само списъци и при речник се отказваше МЪЛЧАЛИВО: корпусът
        # се обхождаше, отчетът казваше „0 текста" и изглеждаше, че думата
        # просто я няма. Открито при `titles_bg.json` на словата, който е
        # плосък речник id → заглавие. (18.09.2026)
        if isinstance(възел, dict):
            двойки = list(възел.items())
        elif isinstance(възел, list):
            двойки = list(enumerate(възел))
        else:
            return
        for i, дете in двойки:
            белег = ('[%d]' % i) if isinstance(i, int) else ('.%s' % i)
            if остатък:
                ...
                if нов != дете:
                    възел[i] = нов
                    промени.append((адрес + белег, дете, нов))
        return
...
КОРПУСИ = [
    ...
    КорпусJSON('optina',
               ...
               [('body_bg',)]),
    # ⚠ Словата бяха ЕДИНСТВЕНИЯТ преведен корпус извън този списък, тъй че
    # всяка масова поправка ги подминаваше МЪЛЧАЛИВО. Открито при замяната
    # „четвородневния" → „четверодневния" (18.09.2026): думата седеше в две
    # слова и щеше да остане там.
    #
    # ⚠ И тук се пипат ДВЕ неща — изворът (`work/translated/`) и готовата
    # база, — по същата причина както при `lives.db`: пътят от извора до
    # базата минава през стъпки, които теглят наново от azbyka.ru.
    КорпусJSON('slova',
               ('tools', 'lives_plus', 'work', 'translated', '*.json'),
               [('blocks_bg', '*')]),
    # ⚠ Заглавията на словата живеят в СВОЙ файл (прави ги
    # `02b_translate_titles.py`), не в `work/translated/`. Без този ред
    # поправка в заглавие се нанася в базата и се губи при първото
    # пресглобяване.
    КорпусJSON('slova-titles',
               ('tools', 'lives_plus', 'work', 'titles_bg.json'),
               [('*',)]),
    КорпусSQLite('slova-db', _път('assets', 'db', 'lives_plus.db'),
                 'slova', ['body', 'title_bg'], 'id'),
    # ⚠ Сказанието за Пасха и заглавията на връзките към нея. Изворът им е
    # `tools/paskha/work/`; без тези редове всяко пускане на `06_apply.py`
    # връща непоправения текст в базата.
    КорпусJSON('paskha',
               ('tools', 'paskha', 'work', 'text_bg.json'),
               [('*', 'title_bg'), ('*', 'blocks_bg', '*')]),
    КорпусJSON('paskha-links',
               ('tools', 'paskha', 'work', 'links_bg.json'),
               [('*',)]),
    # ⚠⚠ СТАТИИТЕ ОТ azbyka.ru (секцията с понятията около празниците) бяха
    # ИЗВЪН този списък — същият пропуск като при словата и със същата цена:
    # масова поправка ги подминава мълчаливо, а те са петдесет и една статии
    # и над 700 000 знака. ⚠ `sections` е РЕЧНИК („0", „1", …), не списък —
    # обхождането с „*" минава и през двата (виж _обходи_пътека).
    КорпусJSON('paskha-pages',
               ('tools', 'paskha', 'work', 'pages_bg', '*.json'),
               [('title_bg',), ('sections', '*', 'title_bg'),
                ('sections', '*', 'blocks_bg', '*')]),
